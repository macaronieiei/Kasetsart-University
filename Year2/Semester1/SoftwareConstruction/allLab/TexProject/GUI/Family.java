...
public class Family extends JFrame {
    ...
    private int calculateChildDeduction() {
        ...
        if (MHM_havekid.isSelected()) {
            int firstChildDeduction = 30000;
            int MHM_text_before_int = parseIntSafely(MHM_text_before.getText());
            int MHM_text_after_int = parseIntSafely(MHM_text_after.getText());
            return firstChildDeduction + (MHM_text_before_int * 30000) + (MHM_text_after_int * 60000);
        }
        if (MDHM_havekid.isSelected()) {
            int firstChildDeduction = 30000;
            int MDHM_text_before_int = parseIntSafely(MDHM_text_before.getText());
            int MDHM_text_after_int = parseIntSafely(MDHM_text_after.getText());
    ...
    private void addSingleListeners() {
        ...
        //ปุ่มกลม (ญาติ)
        single_cousin.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                updateTotalDeduction();
            }
